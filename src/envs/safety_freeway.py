...
COST_COLLISION = jnp.float32(1.0)
COST_TRESHOLD = jnp.float32(3.0)
...
class State(core.State):
    ...
    # --- Safety MinAtar Freeway specific ---
...
class SafetyMinAtarFreeway(core.Env):
    def __init__(
        self,
        *,
...
def _step_det(
    ...
    pos, cost, cars = _update_cars(pos, cars)

    # Update various timers
    move_timer = jax.lax.cond(move_timer > 0, lambda: move_timer - 1, lambda: move_timer)
    terminate_timer -= ONE
...
def _update_cars(pos, cars):
    def _update_stopped_car(pos, cost, car):
        car = car.at[2].set(jax.lax.abs(car[3]))
        car = jax.lax.cond(car[3] > 0, lambda: car.at[0].add(1), lambda: car.at[0].add(-1))
        car = jax.lax.cond(car[0] < 0, lambda: car.at[0].set(9), lambda: car)
        car = jax.lax.cond(car[0] > 9, lambda: car.at[0].set(0), lambda: car)

        pos, cost = jax.lax.cond(
            (car[0] == 4) & (car[1] == pos), lambda: (NINE, cost + COST_COLLISION), lambda: (pos, cost)
        )
        return pos, cost, car

    def _update_car(carry, car):
        pos, cost = carry

        # If this condition holds we were hit by a car, which incurs a cost
        pos, cost = jax.lax.cond(
            (car[0] == 4) & (car[1] == pos), lambda: (NINE, cost + COST_COLLISION), lambda: (pos, cost)
        )
        pos, cost, car = jax.lax.cond(
            car[2] == 0,
            lambda: _update_stopped_car(pos, cost, car),
            lambda: (pos, cost, car.at[2].add(-1)),
        )
        return (pos, cost), car

    (pos, cost), cars = jax.lax.scan(_update_car, (pos, jnp.float32(0.0)), cars)

    return pos, cost, cars
...
def _random_speed_directions(rng):
    ...
    directions = jax.random.choice(rng2, jnp.array([-1, 1], dtype=jnp.int32), [8])
