...
class ConstraintEpistemicMinatarAZNet(hk.Module):
    """Constraint Epistemic AlphaZero NN architecture for Minatar. Linear layers hidden sizes are all the same, defaults to 64.
    Body:
        One Conv2D with 16 channels followed by two linear layers.
    Policy heads:
        Two linear layers, Relu activations, no activate on last layer.
    Value heads:
        Two linear layers, Relu activations, tanh activate on value
    Cost heads:
        Two linear layers, Relu activations, tanh activate on cost
    """
    ...
    def __init__(
        ...
        cost_scale = Used to scale the cost, which is learned for stability between -1 and 1
